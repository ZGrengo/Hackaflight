...
const FavoritesPage = () => {
    ...
    return (
        <main className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
            <h2 className="text-3xl font-semibold text-center text-dark-blue mb-6">Lista de Búsquedas Favoritas</h2>
            
            {favorites.length === 0 ? (
                <p className="text-center text-gray-500">No tienes búsquedas favoritas aún.</p>
            ) : (
                <ul className="space-y-4">
                    {favorites.map((favorite) => (
                        <li key={favorite.favoriteId} className="bg-gray-100 p-4 rounded-lg shadow-sm">
                            <p className="text-lg font-medium text-gray-800">
                                {favorite.title}: Desde <span className="text-medium-blue">{favorite.origin}</span> a <span className="text-medium-blue">{favorite.destination}</span>
                            </p>
                            <p className="text-sm text-gray-600">
                                Fecha de salida: {new Date(favorite.departureDate).toLocaleDateString("es-ES", { day: "2-digit", month: "long", year: "numeric" })} | Adultos: {favorite.adults}
                            </p>
